...
| 特性 | 说明 |
| --- | --- |
| **新增** 项目自动识别| 通过路径关键词自动匹配 `project_versions.json` 配置，方便扩展新项目 |
| **新增**新固件版本提取 | **自动扫描 `.map` 文件解析版本符号地址，从 `.s19`/`.hex` 中提取版本 ID 和字符串** |
| **新增**AREA转储 | 自动将 TRACE32 AREA 窗口内容保存到log日志文件，同时实时打印到终端 |
| **修复UCB刷写的bug** | **优化了刷写UCB的判断逻辑，修复了UCB刷写过程中可能出现的版本提取错误** |
| **优化终端输出信息** |增加了分色显示:项目名(青色)、固件版本(蓝色)、芯片型号(黄色)、AREA 输出(绿色) |
| **稳定性监控删改** |删除了侵入式的PC指针查看，仅根据调用TRACE的STATE("RUN")状态命令，简要判断运行状态是否正常|
| **全量log日志优化** | 自动在 `logdir/` 按时间戳生成通讯日志，记录每次 `cmd`/`fnc` 调用 |


## 一些问题
1.当前烧录流程存在一个明显限制：由于 TRACE32 在烧录阶段可能处于阻塞状态，脚本无法在烧录进行时实时获取烧录进度信息，导致终端无法同步显示当前进度，难以及时获知烧录是否已经完成、卡在哪个阶段。


2.另一个希望增加的能力是烧录版本的回溯。由于不同项目的版本信息地址通常保持一致，可以考虑在每次烧录完成后输出一份固件版本信息文件，并在下一次烧录前先从这个文件中读取上一次烧录的版本信息。但前提是前一次烧录和测试也是使用本环境完成的，就能快速获知当前待烧录固件与上次版本的关系，方便做版本对比、回溯和问题定位。
...
> **环境独立性验证**：为了验证本环境是否能够独立于具体项目和芯片复用，我使用了 CCU 项目的 TC387QP 芯片进行测试。结果表明，环境在该项目上同样可以完成烧录，并且烧录成功。
...
| 类型 | 项目 | 芯片型号 |
| --- | --- | --- |
| 环境开发依托项目 | VCUPLUS | 英飞凌 TriCore TC397X |
| 测试项目 | CCU | 英飞凌 TriCore TC387QP |
